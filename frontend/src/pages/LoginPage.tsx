    ...
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-base)' }}>
      ...
      <div style={{
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: '16px', marginBottom: '40px', lineHeight: 1.6 }}>
          Secure your LLM applications against prompt injections, jailbreaks, and sensitive data leakage with continuous adversarial testing.
        </p>
        
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', flex: 1 }}>
          {['Prompt Injection Testing', 'Jailbreak Detection', 'Data Leakage Scanning'].map(feature => (
            <div key={feature} style={{ display: 'flex', alignItems: 'center', gap: '12px', color: 'var(--text-primary)', fontSize: '15px' }}>
              <span style={{ color: 'var(--red)' }}>▸</span> {feature}
            </div>
          ))}
        </div>

        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px' }}>
        <div style={{ width: '100%', maxWidth: '400px' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-secondary)' }}>
              <input type="checkbox" checked={persistent} onChange={e => setPersistent(e.target.checked)} style={{ accentColor: 'var(--red)' }} />
              PERSISTENT SESSION
            </label>
            ...
            >
              {loading ? 'AUTHENTICATING...' : 'AUTHENTICATE'} {!loading && <ChevronRight size={18} />}
            </button>
          </form>

          <div style={{ marginTop: '32px', textAlign: 'center' }}>
            <Link to="/register" style={{ color: 'var(--text-muted)', fontSize: '14px', textDecoration: 'underline' }}>
              New operator? Request Access / Register
            </Link>
          </div>
        </div>
      </div>
